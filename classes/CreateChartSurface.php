 ...
 */
class CreateChartSurface extends CreateChartElement
{
    /**
    ...
    public function createEmbeddedXmlChart()
    {
        ...
        $this->generateSCALING();
        $this->generateDELETE($this->delete);
        if (!empty($this->orientation) && is_array($this->orientation) && isset($this->orientation[2]) && !is_null($this->orientation[2]))  {
            $this->generateORIENTATION($this->orientation[2]);
        } else {
            $this->generateORIENTATION();
        }
        if (!empty($this->axPos) && is_array($this->axPos) && isset($this->axPos[2]) && !is_null($this->axPos[2]))  {
            $this->generateAXPOS($this->axPos[2]);
        } else {
        ...
        if (!is_array($this->tickLblPos)) {
            $this->generateTICKLBLPOS();
        } else if (!empty($this->tickLblPos) && is_array($this->tickLblPos) && isset($this->tickLblPos[2]) && !is_null($this->tickLblPos[2])) {
            $this->generateTICKLBLPOS($this->tickLblPos[2]);
        }
        $this->generateCROSSAX(59034624);
        $this->generateCROSSES();
