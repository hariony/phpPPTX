...
class FrameParser_Tree {
  ...
  function build_tree($filter = '*') {
    ...
    if(strpos($filter, '/') === 0){ //xpath expression
        ...
        if($entradas !== false){
            foreach($entradas as $entrada){
                ...
                $entrada->setAttribute('class', ($entrada->hasAttribute('class')?$entrada->getAttribute('class').' ':'').'_phppptx_filter_paint_');
            }
    ...
    if($filter == '*') @$this->_dom->getElementsByTagName("html")->item(0)->setAttribute('class', '_phppptx_filter_paint_');
  ...
  protected function _build_tree_r(DOMNode $node) {
    if (file_exists(__DIR__ . '/HTMLExtended.php') && CreateHtml::$htmlExtended) {
    ...
    switch($node->nodeName){
      ...
      case 'div':
        ...
        if($this->parseDivs == 'table'/* && $nodeTable*/){
          ...
          $filter = ($filter == '*' || (isset($aDompdfTree['attributes']['class']) && in_array('_phppptx_filter_paint_', $aDompdfTree['attributes']['class'])))?'*':false;
...
class StyleParser {
  ...
  function length_in_pt($length, $ref_size = null) {
    if (CreateHtml::$parseCSSVars && isset($length)) {
  ...
  function munge_colour($colour) {
    ...
    // parse CSS variables
    if (CreateHtml::$parseCSSVars && substr($colour, 0, 4) == 'var(') {
      return $colour;
    }
  }
  ...
  function getpropstyle($prop) {
    ...
    if (CreateHtml::$parseCSSVars && isset($this->_props[$prop])) {
  ...
  function get_props($prop) {
    // parse CSS variables
    if (CreateHtml::$parseCSSVars && $prop == 'font_family' && (isset($this->_props[$prop]) || isset($this->_prop_cache[$prop]))) {
      $newFontFamily = false;
